...
export type LiveTransaction = {
  ...
  graph?: {
    card_id?: number;
    kyc_ssn_reuse: number;
    kyc_email_match: boolean;
    kyc_phone_reuse: number;
    addr_per_card: number;
    d1_days: number;
    device_type: string;
  };
  features?: {
    card: Record<string, number | null>;
    velocity: Record<string, number | null>;
    time_delta: Record<string, number | null>;
    match_flags: Record<string, number | null>;
    device: Record<string, number | null>;
    identity: Record<string, number | null>;
    kyc: Record<string, number | null>;
  };
...
const WS_URL = (import.meta.env.VITE_WS_URL as string | undefined) ?? 'ws://127.0.0.1:8002/ws/live';
...
export default function useLiveStream() {
  const { transactions, stats, clear } = useLiveStreamStore();
