...
def _generate_insight(alert: dict) -> dict:
    """Generate rule-based fraud insight from alert data."""
    tx_type = alert.get("transaction_type", "UNKNOWN")
    amount = alert.get("amount", 0)
    prob = alert.get("fraud_probability", 0)
    old_orig = alert.get("old_balance_orig", 0)
    new_orig = alert.get("new_balance_orig", 0)
    old_dest = alert.get("old_balance_dest", 0)
    new_dest = alert.get("new_balance_dest", 0)
    ...
    signals = []
    if amount > 5000:
        signals.append({"name": "High Transaction Amount", "severity": "HIGH",
                        "detail": f"Amount ${amount:,.2f} exceeds typical threshold of $5,000"})
    if old_orig > 0 and new_orig == 0:
        signals.append({"name": "Full Balance Drain", "severity": "CRITICAL",
                        "detail": f"Sender balance wiped from ${old_orig:,.2f} to $0.00"})
    if tx_type in ("CASH_OUT", "TRANSFER") and prob >= 0.7:
        signals.append({"name": "High-Risk Transaction Type", "severity": "HIGH",
                        "detail": f"{tx_type} transactions are the most common fraud vector"})
    if old_dest == 0 and new_dest > 0:
        signals.append({"name": "New Destination Account", "severity": "MEDIUM",
                        "detail": "Funds moved to a previously zero-balance destination account"})
    if prob >= 0.9:
        signals.append({"name": "Extreme Fraud Probability", "severity": "CRITICAL",
                        "detail": f"Model confidence {prob*100:.1f}% — far above the 50% fraud threshold"})
    if not signals:
        signals.append({"name": "Anomalous Pattern", "severity": "MEDIUM",
                        "detail": "Transaction deviates from expected behaviour for this account"})

    what_happened = (
        f"A {tx_type} of ${amount:,.2f} was flagged with {prob*100:.1f}% fraud probability. "
        f"Sender balance changed from ${old_orig:,.2f} to ${new_orig:,.2f}."
    )
    why_fraud = (
        f"The combination of transaction type ({tx_type}), amount, and balance movement "
        f"matches known fraud patterns in the training data. Risk level: {risk}."
    )
    action = "Freeze Transaction" if prob >= 0.85 else "Escalate for Review" if prob >= 0.65 else "Flag for Manual Review"

    return {
        "source": "model",
        "fraud_report": {
            "title": f"Fraudulent {tx_type} detected",
            "risk_level": risk,
...
def get_llm_report_for_tx(round: int, transaction_index: int):
    """Returns the insight for a specific transaction — stored or auto-generated."""
    with engine.connect() as conn:
        row = conn.execute(
    ...
    # No stored insight — find the matching alert and auto-generate
    match = next((a for a in _fraud_alerts if a.get("transaction_index") == transaction_index and a.get("round") == round), None)
    if match is None:
        # Fall back to any alert with matching index
        match = next((a for a in _fraud_alerts if a.get("transaction_index") == transaction_index), None)
    if match is None:
        return None
    return _generate_insight(match)
